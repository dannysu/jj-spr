...
fn test_master_base_identification_for_stacked_commits() {
    ...
    fs::write(repo_path.join("master1.txt"), "master content 1").expect("Failed to write master file");
    ...
    if !master1_output.status.success() {
        panic!("Failed to create master commit 1: {}", String::from_utf8_lossy(&master1_output.stderr));
    }
    ...
    let master_base_change_id = String::from_utf8_lossy(&master_base_change_id_output.stdout).trim().to_string();
    ...
    let master_base_oid_output = Command::new("jj")
        .args(["log", "-r", &master_base_change_id, "--no-graph", "-T", "commit_id"])
        .current_dir(repo_path)
        .output()
        .expect("Failed to get master base OID");
    let master_base_oid = String::from_utf8_lossy(&master_base_oid_output.stdout).trim().to_string();
    ...
    if !merge_base_output.status.success() {
        panic!("Failed to calculate merge base: {}", String::from_utf8_lossy(&merge_base_output.stderr));
    }
    
    let calculated_merge_base = String::from_utf8_lossy(&merge_base_output.stdout).trim().to_string();
    println!("Calculated merge base: {}", calculated_merge_base);
    
    // The merge base should be the master base OID, not the immediate parent
    assert_eq!(
        calculated_merge_base, 
        master_base_oid,
...
fn test_directly_based_on_master_logic_fix() {
    ...
    println!("Old logic (buggy): directly_based_on_master = {}", directly_based_on_master_old_logic);
    println!("New logic (fixed): directly_based_on_master = {}", directly_based_on_master_new_logic);
    
    // The old logic incorrectly thought the child was directly based on master
    assert!(directly_based_on_master_old_logic, "Old logic should incorrectly return true (this demonstrates the bug)");
    
    // The new logic correctly identifies that the child is NOT directly based on master
    assert!(!directly_based_on_master_new_logic, "New logic should correctly return false for stacked child");
