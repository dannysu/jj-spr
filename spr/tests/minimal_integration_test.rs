...
fn test_binary_exists_and_has_correct_identity() {
    ...
    assert!(output.status.success(), "Binary should respond to --version");
    
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("jj-spr"), "Should identify as jj-spr");
    assert!(stdout.contains("1.3.6-beta.1"), "Should show correct version");
}
...
fn test_help_shows_jujutsu_subcommand_identity() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Jujutsu subcommand"), "Should identify as Jujutsu subcommand");
    
    // Should list all main commands
    let commands = vec!["diff", "format", "land", "amend", "close", "list", "patch", "init"];
    for cmd in commands {
...
fn test_subcommands_have_help() {
    ...
    for (cmd, expected_keyword) in commands {
        ...
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(stdout.to_lowercase().contains(&expected_keyword.to_lowercase()), 
               "Help for {} should mention {}", cmd, expected_keyword);
    }
}
...
fn test_requires_jujutsu_repository() {
    ...
    assert!(!output.status.success(), "Should fail without any repository");
    ...
    let output = run_jj_spr(&["format"], Some(temp_dir.path()));
    assert!(!output.status.success(), "Should fail with only git repository");
...
fn test_configuration_detection() {
    ...
    let output = run_jj_spr(&["format"], Some(&repo_path));
    assert!(!output.status.success(), "Should fail without configuration");
...
fn test_commands_fail_appropriately_without_config() {
    ...
    for cmd in github_commands {
        ...
        assert!(!output.status.success(), "Command {} should fail without config", cmd);
        ...
        assert!(
            ...
            all_output.contains("not found") ||
            all_output.contains("githubRepository"),
            "Command {} should fail due to missing configuration", cmd
        );
    }
}
