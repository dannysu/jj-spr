...
fn test_stacked_pr_shows_only_child_diff() {
    ...
    if !git_init_output.status.success() {
        panic!("Failed to initialize git repo: {}", String::from_utf8_lossy(&git_init_output.stderr));
    }
    ...
    if !init_output.status.success() {
        panic!("Failed to initialize jj repo: {}", String::from_utf8_lossy(&init_output.stderr));
    }
    
    // Create an initial commit on master
    fs::write(repo_path.join("initial.txt"), "initial content").expect("Failed to write initial file");
    ...
    if !initial_commit_output.status.success() {
        panic!("Failed to create initial commit: {}", String::from_utf8_lossy(&initial_commit_output.stderr));
    }
    ...
    if !parent_commit_output.status.success() {
        panic!("Failed to create parent commit: {}", String::from_utf8_lossy(&parent_commit_output.stderr));
    }
    ...
    let parent_change_id_before = String::from_utf8_lossy(&parent_change_id_output.stdout).trim().to_string();
    ...
    if !child_commit_output.status.success() {
        panic!("Failed to create child commit: {}", String::from_utf8_lossy(&child_commit_output.stderr));
    }
    ...
    let child_tree_output = Command::new("jj")
        .args(["log", "-r", &child_change_id, "--no-graph", "-T", "commit_id"])
        .current_dir(repo_path)
        .output()
        .expect("Failed to get child commit ID");
        
    let child_commit_id = String::from_utf8_lossy(&child_tree_output.stdout).trim().to_string();
    ...
    let parent_tree_output = Command::new("jj")
        .args(["log", "-r", &parent_change_id_before, "--no-graph", "-T", "commit_id"])
        .current_dir(repo_path)
        .output()
        .expect("Failed to get parent commit ID");
        
    let parent_commit_id = String::from_utf8_lossy(&parent_tree_output.stdout).trim().to_string();
    ...
    let parent_change_id_after_output = Command::new("jj")
        .args(["log", "-r", &parent_commit_id, "--no-graph", "-T", "change_id"])
        .current_dir(repo_path)
        .output()
        .expect("Failed to get parent change ID after");
        
    let parent_change_id_after = String::from_utf8_lossy(&parent_change_id_after_output.stdout).trim().to_string();
    ...
    let modify_parent_output = Command::new("jj")
        .args(["describe", "-r", &parent_change_id_before, "-m", "Parent commit for stacking (modified)"])
        .current_dir(repo_path)
        .output()
        .expect("Failed to modify parent commit");
        
    if !modify_parent_output.status.success() {
        panic!("Could not modify parent commit - it became immutable: {}", 
...
fn test_jj_spr_stacked_behavior_simulation() {
    ...
    let parent_change_id_output = Command::new("jj")
        ...
        .output()
        .expect("Failed to get parent change ID");
    let parent_change_id = String::from_utf8_lossy(&parent_change_id_output.stdout).trim().to_string();
    ...
    let child_oid = String::from_utf8_lossy(&child_oid_output.stdout).trim().to_string();
    
    // 2. Get parent OID for the child commit (this is what might cause issues)
    let parent_oid_output = Command::new("jj")
        .args(["log", "-r", &parent_change_id, "--no-graph", "-T", "commit_id"])
        .current_dir(repo_path)
        .output()
        .expect("Failed to get parent OID");
    let parent_oid = String::from_utf8_lossy(&parent_oid_output.stdout).trim().to_string();
    ...
    let parent_change_id_from_oid = String::from_utf8_lossy(&parent_change_id_from_oid_output.stdout).trim().to_string();
